...
public class InvoiceSlipPdfView extends AbstractPdfView {
	...
	protected void buildPdfMetadata(Map<String, Object> model, 
		...
		HeaderFooter footer = new HeaderFooter(new Phrase(new Date() + " (C) DGati Soft, Page # "), true);
	...
	protected void buildPdfDocument(Map<String, Object> model, 
		...
		Paragraph invoiceId = new Paragraph(
				sr.getPatient().getFirstName() + " " + sr.getPatient().getLastName() + "-" + System.currentTimeMillis(),
				invoiceFont);
		...
		// adding Image
		Image img = Image.getInstance("/static/images/hms.jpg");
		...
		double fee = sr.getAppointment().getConsultationFee();
		...
		table.addCell(getTextCell(sr.getPatient().getFirstName() + " " + sr.getPatient().getLastName()));
		...
		billdata.addCell(sr.getAppointment().getConsultationFee().toString());
		...
		Paragraph note = new Paragraph(
				"NOTE: THIS IS AUTO-GENERATED PAYMENT SLIP, FOR MORE DETALS CONTACT FRONTDESK @ 1234567890", noteFont);
